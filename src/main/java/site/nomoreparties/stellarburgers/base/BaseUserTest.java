...
import site.nomoreparties.stellarburgers.actions.UserAction;
import site.nomoreparties.stellarburgers.constants.UserFields;
import site.nomoreparties.stellarburgers.generating.GenerateUserData;
...
public class BaseUserTest extends UserAction {
    ...
    public String changeUserCardValueToRandom(String updatedField) {
        ...
        switch (updatedField) {
            case UserFields.EMAIL:
                fieldValue = generateUserData.getUserEmail();
                break;
            case UserFields.PASSWORD:
                fieldValue = generateUserData.getUserPassword();
                break;
            case UserFields.NAME:
                fieldValue = generateUserData.getUserName();
                break;
        }
    ...
    public void changeUserCardValue(String userField, String fieldValue) {
        switch (userField) {
            case UserFields.EMAIL:
                userCard.setEmail(fieldValue);
                break;
            case UserFields.PASSWORD:
                userCard.setPassword(fieldValue);
                break;
            case UserFields.NAME:
                userCard.setName(fieldValue);
                break;
        }
    }
}
